...
public final class JTCompilationUnit
    ...
    public JTCompilationUnit analyze() {
        setPackage(Optional.ofNullable(getOriginalTree().getPackage())
                .map(p -> new JTPackageDecl(p, this))
                .map(JTTree::analyze)
                .orElse(null));
        return this;
    }
    ...
    public JTCompilationUnit save(Path outputPath) throws IOException {
        return save(outputPath, StandardCharsets.UTF_8);
    }

    public JTCompilationUnit save(Path outputPath, Charset charset) throws IOException {
    ...
    @Override
    public JTCompilationUnit save(Writer writer) throws IOException {
        if (isDirty()) {
            throw new UnsupportedOperationException();
        }
        writer.write(getOriginalCode());
        return this;
    }

    public JTCompilationUnit setPackage(JTPackageDecl packageTree) {
