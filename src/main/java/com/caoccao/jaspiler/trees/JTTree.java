...
public abstract class JTTree<
    ...
    protected boolean replaceCatches(List<JTCatch> list, V8Value v8Value) throws JavetException {
        if (v8Runtime.toObject(v8Value) instanceof List<?> trees) {
            list.clear();
            trees.stream()
                    .filter(tree -> tree instanceof JTCatch)
                    .map(tree -> ((JTCatch) tree).setParentTree(this))
                    .forEach(list::add);
            setActionChange();
            return true;
        }
    ...
    protected boolean replaceKind(Function<Kind, NewTree> setter, V8Value v8Value) {
        if (v8Value instanceof V8ValueString v8ValueString) {
            setter.apply(Kind.valueOf(v8ValueString.getValue()));
            return true;
        }
        return false;
    }
    ...
    protected boolean replacePattern(Function<JTPattern<?, ?>, NewTree> setter, V8Value v8Value) throws JavetException {
        if (v8Runtime.toObject(v8Value) instanceof JTPattern<?, ?> tree) {
            setter.apply(tree);
            return true;
        }
        return false;
